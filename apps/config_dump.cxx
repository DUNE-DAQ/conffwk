...
// Renamed since fork: yes (from bin/config_dump.cpp to apps/config_dump.cxx).
//
...
ERS_DECLARE_ISSUE(config_dump, BadCommandLine, "bad command line: " << reason, ((const char*)reason))

ERS_DECLARE_ISSUE(config_dump, ConfigException, "caught dunedaq::conffwk::Exception exception", )

struct SortByName
{
  bool operator()(const ConfigObject* o1, const ConfigObject* o2) const { return o1->UID() < o2->UID(); }
};
...
static void
print_versions(const std::vector<dunedaq::conffwk::Version>& versions)
{
...
main(int argc, char* argv[])
{
  ...
  dunedaq::conffwk::Version::QueryType query_type = dunedaq::conffwk::Version::query_by_tag;
  ...
  boost::program_options::options_description desc("Dumps class and objects descriptions using abstract conffwk API.\n"
  ...
  try {
    ...
    desc.add_options()("database,d",
      ...
      "list-objects,o", "list objects of class")("print-referenced-by,r",
      ...
      "show-contained-in,n", "when dump an object, print out the database file it belongs to")("help,h",
                                                                                               "print help message");
    ...
    if (!vesrions_str.empty()) {
      if (vesrions_str.size() != 4) {
        ...
        if (vesrions_str[1] == "date")
          query_type = dunedaq::conffwk::Version::query_by_date;
        else if (vesrions_str[1] == "id")
          query_type = dunedaq::conffwk::Version::query_by_id;
    ...
    if (object_id == any) {
      ...
      for (std::set<std::string>::const_iterator i = classes.begin(); i != classes.end(); ++i) {
        ...
        if (objects.empty()) {
          ...
          std::cout << prefix << "The class \'" << *i << "\' has " << objects.size()
