...
// Renamed since fork: yes (from bin/config_export_data.cpp to apps/config_export_data.cxx).
//
...
main(int argc, char* argv[])
{
  ...
  try {
    desc.add_options()("database,d",
      ...
      "classes,c", boost::program_options::value<std::string>(&classes), "regex defining class names; ignore if empty")(
      "objects,i", boost::program_options::value<std::string>(&objects), "regex defining object IDs; ignore if empty")(
      "files,f", boost::program_options::value<std::string>(&files), "regex defining data files; ignore if empty")(
      "output,o",
...

    {
      ...
      if (format == "json")
        ...
        boost::property_tree::xml_parser::write_xml(
          buf, pt, boost::property_tree::xml_writer_make_settings<std::string>(' ', 4));
      else
