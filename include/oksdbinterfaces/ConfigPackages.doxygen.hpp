  /**
   *  \file ConfigPackages.doxygen.h This file contains the main page for the oksdbinterfaces package documentation.
   *  \author Igor Soloviev
   *  \brief oksdbinterfaces documentation file 
   */

#ifndef OKSDB_INTERFACE_DOXYGEN_H_
#define OKSDB_INTERFACE_DOXYGEN_H_

/** \page ConfigPackages Config Packages

The goal of the oksdbinterfaces package %is to provide user-friendly API to access %data from the configuration database.
There are two layers of such API which can be seen by user:
\li abstract oksdbinterfaces layer working with arbitrary database schema and hiding details of DBMS implementation
...
existing %data using generated DAL.
  ...
  \li \ref DalGeneration
      <ul>
      <li> \ref GenoksdbinterfacesParams
...
A DAL %is generated by the genoksdbinterfaces utility. It uses OKS schema files as input and produces:
...
\li genoksdbinterfaces.info file containing information about names of generated classes,
    the C++ code namespace, include prefix directory and java package name


  \subsection GenoksdbinterfacesParams 2.1. Parameters of genoksdbinterfaces utility

The command line parameters of genoksdbinterfaces utility are listed below:
\code{.txt}
genoksdbinterfaces [-d | --C++-dir-name directory-name]
...
Methods of C++ and Java oksdbinterfaces classes throw exceptions in case of errors.
...
\li \c daq::oksdbinterfaces::Generic        %is used to report most of the problems (bad DB, wrong parameter, plug-in specific, etc.)
\li \c daq::oksdbinterfaces::NotFound       the oksdbinterfaces object accessed by ID %is not found, class accessed by name %is not found
\li \c daq::oksdbinterfaces::DeletedObject  accessing template object that has been deleted (via notification or by the user's code)

All above exceptions have common class \c daq::oksdbinterfaces::Exception, that can be used to catch all of them.
\code{.cpp}
try {
...
catch (daq::oksdbinterfaces::Exception & ex) {
    // throw some user-defined exception in case of oksdbinterfaces exception
  throw ers::error(user::exception(ERS_HERE, "cannot read oksdbinterfaces", ex));
}
\endcode
...
and in the oksdbinterfaces package in Java. Below it %is described how to create an object of this class and how
...
then the configuration constructor throws \c daq::oksdbinterfaces::Generic in case of C++,
or \c oksdbinterfaces.SystemException %is thrown in case of Java.
...
\li the \b rdboksdbinterfaces accessing OKS with RDB %server.
...
try {
  ...
  ::Configuration db4("rdboksdbinterfaces");
...
try {
  oksdbinterfaces.Configuration db = new oksdbinterfaces.Configuration("rdboksdbinterfaces:RDB");
}
catch (oksdbinterfaces.SystemException ex) {
  System.err.println( "ERROR caught \'oksdbinterfaces.SystemException\':");
...
\li using the same approach as for C++, i.e. via constructor parameter using double colon-separated partition and %server names,
  e.g. \e "rdboksdbinterfaces:partition-name::server-server" or \e "rdboksdbinterfaces:server-server@partition-name";
...
should be used. The usage of oksdbinterfaces layer (i.e. direct usage of objects of ConfigObject class) only makes sense
...
\li Java methods generated in class T_Helper:
  <ul>
  <li> static public %T get(oksdbinterfaces.Configuration db, String id) to read named object
  <li> static public %T[] gets(oksdbinterfaces.Configuration db, Query query) to read objects of class
  </ul>
...
...
oksdbinterfaces.Configuration db = new oksdbinterfaces.Configuration(...);
try {
  dal.Application objs[] = dal.Application_Helper.get(db, new oksdbinterfaces.Query());
} catch ( oksdbinterfaces.NotFoundException ex ) {
...
...
dal.RunControlApplication objs[] = dal.RunControlApplication_Helper.get(db, new oksdbinterfaces.Query());
\endcode

Below there %is an example of code to get an application by ID.
Note, if object with such ID does not exist, oksdbinterfaces.NotFoundException exception %is thrown.
\code{.java}
try {
  dal.Application obj = dal.Application_Helper.get(db, "RootController");
}
catch (oksdbinterfaces.NotFoundException e) {
...
In case of Java a converter class has to implement  oksdbinterfaces.AttributeConverter interface
...
\code{.java}
import oksdbinterfaces.Configuration;
import oksdbinterfaces.AttributeConverter;

  // converter removes leading and trailing whitespace from a string
public class TrimString implements oksdbinterfaces.AttributeConverter {
  public Object convert(Object s, oksdbinterfaces.Configuration db, oksdbinterfaces.ConfigObject obj, String attr_name) { return (Object)(s.trim()); }
...
\code{.java}
oksdbinterfaces.Configuration db = new oksdbinterfaces.Configuration("rdboksdbinterfaces"); // no db file
\endcode
...
\code{.cpp}
try {
  ::Configuration db("rdboksdbinterfaces");
  std::list<std::string> includes;
  includes.push_back("online/schema/online.schema.xml"); // common schema
  includes.push_back("online/segments/setup.data.xml");  // online infrastructure
  const char * db_name = "/tmp/my-partition.data.xml";   // new database file name
...
\code{.java}
try {
  oksdbinterfaces.Configuration db = new oksdbinterfaces.Configuration("rdboksdbinterfaces");
  ...
  System.err.println("ERROR: caught \'oksdbinterfaces.System\' exception");
}
catch(oksdbinterfaces.NotAllowedException ex) {
  System.err.println("ERROR: caught \'oksdbinterfaces.NotAllowed\' exception");
}
... // catch oksdbinterfaces.AlreadyExistsException in a similar way
\endcode
...
Similar methods in Java class oksdbinterfaces.Configuration are:
...
\li <tt> %T create(oksdbinterfaces.Configuration db, String at, String id) </tt> -
    to create new object of class %T with identity id at existing database file with name at;
\li <tt> %T create(oksdbinterfaces.Configuration db, oksdbinterfaces.DalObject at, String id) </tt> -
    to create new object of class %T with identity id at a database file where object at %is stored.

The example to create online segment and it's application %is shown below::
\code{.java}
oksdbinterfaces.Configuration db = new ...
...
In Java the %method void destroy(oksdbinterfaces.Configuration db) %is generated in T.java, e.g.:
\code{.java}
oksdbinterfaces.Configuration db = new ...;
...
In C++ such %method throws daq::oksdbinterfaces::Exception if failed:
...
For C++ it has the following format and throws daq::oksdbinterfaces::Exception if failed:
...
a %method invoked on corresponding DAL objects will throw daq::oksdbinterfaces::DeletedObject exception,
...
for(auto& i : nodes) {
  ...
  catch(const daq::oksdbinterfaces::DeletedObject&) {
...
::Configuration::notify %callback function in C++ or oksdbinterfaces.Callback interface on Java. 
...
The ::ConfigurationChange class %is declared in the oksdbinterfaces/Change.h file and has 4 methods 
...
The user has to create a class implementing the oksdbinterfaces.Callback interface. It requires to 
implement %method void process_changes(oksdbinterfaces.Change[] changes, java.lang.Object parameter). 
Example below illustrates how to implement notification %callback:
\code{.java}
class TestCallback implements oksdbinterfaces.Callback {
  private oksdbinterfaces.Configuration db;

  public TestCallback(oksdbinterfaces.Configuration d) { db = d; }

  public void process_changes(oksdbinterfaces.Change[] changes, java.lang.Object parameter) {
    ...
    oksdbinterfaces.Change.print(changes, "  ");

      // iterate changes by classes
    for(int i = 0; i < changes.length; i++) {
      oksdbinterfaces.Change change = changes[i];
...
oksdbinterfaces.Subscription class in Java. It %is used to define lists of classes and objects, which 
...
For a class generated by genoksdbinterfaces the s_class_name attribute can be used, 
...
In Java %method oksdbinterfaces.Subscription.add(String class_name) should be used, 
e.g. to subscribe on changes in class Application it %is necessary to write the following code:
\code{.java}
oksdbinterfaces.Subscription s = new oksdbinterfaces.Subscription(new TestCallback(db), null);
s.add("Application");
\endcode
...
In Java oksdbinterfaces.add(DalObject obj) %method to be used, e.g.:
\code{.java}
...
\code{.java}
  // user-defined callback
class MyCallback implements oksdbinterfaces.Callback { ... }

  // configuration object
oksdbinterfaces.Configuration db = new oksdbinterfaces.Configuration(...);

  // subscription criteria object
oksdbinterfaces.Subscription c = new oksdbinterfaces.Subscription(new MyCallback(db), null);
c.add("Application");
...
To be taken into account by genoksdbinterfaces, user has to provide C++ and/or Java implementation. 
...
#endif // OKSDB_INTERFACE_DOXYGEN_H_
