   ...
   *  \brief oksdbinterfaces entry point
   */

#ifndef OKSDB_INTERFACE_CONFIGURATION_H_
#define OKSDB_INTERFACE_CONFIGURATION_H_
...
private:
   ...
   *  by the genoksdbinterfaces utility.
   *
   ...
   *  - dunedaq::oksdbinterfaces::Generic        is used to report most of the problems (bad DB, wrong parameter, plug-in specific, etc.)
   *  - dunedaq::oksdbinterfaces::NotFound       the oksdbinterfaces object accessed by ID is not found, class accessed by name is not found
   *  - dunedaq::oksdbinterfaces::DeletedObject  accessing template object that has been deleted (via notification or by the user's code)
   *
   *  All above exceptions have common class dunedaq::oksdbinterfaces::Exception, that can be used to catch all of them.
   *
\code   
try {
  ...
  // the catch of dunedaq::oksdbinterfaces::NotFound exception is optional:
...
\endcode   
   *
   *  For objects of classes generated by genoksdbinterfaces there are analogous template methods which
...
class Configuration {
  ...
  public:
    ...
    Configuration() = default;
      ...
      /** Get implementation plug-in and it's parameter used to build oksdbinterfaces object */

    const std::string& get_impl_spec() const noexcept {return m_impl_spec;}


      /** Get implementation plug-in name used to build oksdbinterfaces object */
     
    const std::string& get_impl_name() const noexcept {return m_impl_name;}


       /** Get implementation plug-in parameter used to build oksdbinterfaces object */
  ...
  public:
    ...
    CallbackId subscribe(pre_notify user_cb, void * user_param = nullptr);
       ...
       *
       *  \throw dunedaq::oksdbinterfaces::Generic in case of an error (e.g. bad ID, plugin-specific problems)
       */
  ...
  public:
    ...
    void _reset_all_objects() noexcept;
       ...
       *
       *  \param unread_implementation_objs  if true, clear implementation objects; otherwise only template objects are unread and the implementation oksdbinterfaces objects are still valid.
       */
    ...
    void
    unread_implementation_objects(dunedaq::oksdbinterfaces::ObjectState state) noexcept
    {
  ...
  private:
    ...
    static void
    update_impl_objects(oksdbinterfaces::pmap<oksdbinterfaces::map<ConfigObjectImpl *> * >& cache, ConfigurationChange& change, const std::string * class_name);

    void
    _unread_template_objects() noexcept;

    void
    _unread_implementation_objects(dunedaq::oksdbinterfaces::ObjectState state) noexcept;
  ...
  public:
    ...
    void create(const ConfigObject& at, const std::string& class_name, const std::string& id, ConfigObject& object);
       ...
       *
       *  Such method to be used for user classes generated by the genoksdbinterfaces utility.
       *
       *  \param at            database file where to create new object
       *  \param id            object identity
    ...
    template<class T> const T * create(const std::string& at, const std::string& id, bool init_object = false);
       ...
       *  Such method to be used for user classes generated by the genoksdbinterfaces utility.
       *
       *  \param at            an existing object of class generated by genoksdbinterfaces to define location of the file where to store new object
       *  \param id            object identity
  ...
  public:
       ...
       *
       *  \throw dunedaq::oksdbinterfaces::Generic if there is no such class or in case of an error
       */
    ...
    bool test_object(const std::string& class_name, const std::string& id, unsigned long rlevel = 0, const std::vector<std::string> * rclasses = 0);
       ...
       *  \param rclasses     optional array of class names to optimize performance (defines which referenced objects have to be read to the implementation cache)
       *
       *  \throw dunedaq::oksdbinterfaces::NotFound exception if there is no such object or \b dunedaq::oksdbinterfaces::Generic in case of an error
       */
    ...
    void get(const std::string& class_name, const std::string& id, ConfigObject& object, unsigned long rlevel = 0, const std::vector<std::string> * rclasses = 0);
       ...
       *  \param rclasses     optional array of class names to optimize performance (defines which referenced objects have to be read to the implementation cache)
       *
       *  \throw dunedaq::oksdbinterfaces::NotFound exception if there is no such class or \b dunedaq::oksdbinterfaces::Generic in case of an error
       */
  ...
  template<class T>
    ...
    get(const std::string& id, bool init_children = false, bool init = true, unsigned long rlevel = 0, const std::vector<std::string> * rclasses = 0)
    {
       ...
       *  Such method to be used for user classes generated by the genoksdbinterfaces utility.
       *
       *  \param obj            reference to oksdbinterfaces object, that is used to instantiate template object
       *  \param init_children  if true, the referenced objects are initialized
       *  \param init           if true, the object's attributes and relationships are read
       *
  ...
  template<class T>
    ...
    get(ConfigObject& obj, bool init_children = false, bool init = true)
    {
       ...
       *  \param rclasses       optional array of class names to optimize performance (defines which referenced objects have to be read to the implementation cache)
       *
       *  \throw dunedaq::oksdbinterfaces::Generic in case of a problem (e.g. no such class, plug-in specific problem)
       */
  ...
  template<class T>
    ...
    find(const std::string& id)
    {
       ...
       *
       *  The method is used by the code generated by the genoksdbinterfaces utility.
       *
       *  \param obj   object
       ...
       *
       *  \throw dunedaq::oksdbinterfaces::Generic in case of a problem (e.g. no relationship with such name, plug-in specific problem)
       */
  ...
  private:
    ...
    /// \throw dunedaq::oksdbinterfaces::Generic or dunedaq::oksdbinterfaces::NotFound
  ...
  public:
    ...
    void load(const std::string& db_name);
       ...
       *  via oksdbinterfaces and template get methods) and frees all DB resources
  ...
  public:
    ...
    /**
     *  \brief Get new oksdbinterfaces versions.
    ...
    std::vector<dunedaq::oksdbinterfaces::Version>
    get_versions(const std::string& since, const std::string& until, dunedaq::oksdbinterfaces::Version::QueryType type = dunedaq::oksdbinterfaces::Version::query_by_date, bool skip_irrelevant = true);
  ...
  public:
       ...
       *
       *  \throw dunedaq::oksdbinterfaces::NotFound exception if there is no class with such name or \b dunedaq::oksdbinterfaces::Generic in case of a problem
       */

    const dunedaq::oksdbinterfaces::class_t& get_class_info(const std::string& class_name, bool direct_only = false);
  ...
  private:
    ...
    oksdbinterfaces::map<dunedaq::oksdbinterfaces::class_t *> p_direct_classes_desc_cache;
    oksdbinterfaces::map<dunedaq::oksdbinterfaces::class_t *> p_all_classes_desc_cache;
  ...
  public:
     ...
     *  \param  direct_only  if true is set explicitly, return descriptions of direct attributes, relationships, super- and subclasses; by default return all descriptions taking into account inheritance
     *
     *  \throw dunedaq::oksdbinterfaces::Generic in case of a problem
     */
  ...
  private:
    ...
    oksdbinterfaces::fmap<oksdbinterfaces::fset> p_superclasses;
    oksdbinterfaces::fmap<oksdbinterfaces::fset> p_subclasses;
  ...
  public:
    ...
    const oksdbinterfaces::fmap<oksdbinterfaces::fset>& superclasses() const noexcept {return p_superclasses;}


      /** Get names of subclasses for each class **/

    const oksdbinterfaces::fmap<oksdbinterfaces::fset>& subclasses() const {return p_subclasses;}
  ...
  private:
    ...
    oksdbinterfaces::map<std::list<AttributeConverterBase*> * > m_convert_map;
  ...
  public:
       ...
       *  \li <tt> T * get(Configuration&, ConfigObject&, bool, bool) </tt> - get template object for given oksdbinterfaces object
    ...
    template<class T> class Cache : public CacheBase {
      ...
      public:
        ...
        virtual ~Cache() noexcept;
           ...
           /**
            *  \brief Get template object from cache by oksdbinterfaces object.
            *
            *  The method searches an object with id of given oksdbinterfaces object within the cache.
            *  If found, the method sets given oksdbinterfaces object as implementation of the template
	    *  object and returns pointer on the template object.
            *  If there is no such object in cache, then it is created from given oksdbinterfaces object.
            *
            ...
            *
            *  \param oksdbinterfaces         the configuration object
            *  \param obj            the oksdbinterfaces object used to set for the template object
            ...
            *  \return Return pointer to object.
            *
            *  \throw dunedaq::oksdbinterfaces::Generic is no such class for loaded configuration DB schema or in case of an error
            */

        T * get(Configuration& oksdbinterfaces, ConfigObject& obj, bool init_children, bool init_object);
          ...
          T * get(Configuration& oksdbinterfaces, const std::string& name, bool init_children, bool init_object, unsigned long rlevel, const std::vector<std::string> * rclasses);
          ...
          find(const std::string& id);
           ...
           /**
            *  \brief Generate template object using oksdbinterfaces object and ID.
            *
            *  The method searches an object with id of given oksdbinterfaces object within the cache using given ID.
            *  If found, the method sets given oksdbinterfaces object as implementation of the template
            *  object and returns pointer on the template object.
            *  If there is no such object in cache, then it is created from given oksdbinterfaces object.
            *
...
// Instantiate the template parameter using existing oksdbinterfaces object.
template<class T>
  const T *
...
template<class T>
  ...
  Configuration::_ref(ConfigObject& obj, const std::string& name, bool read_children)
  {
    ...
    // std::cout << "- Looking up " << res.UID() << " <" << res.class_name() << "> and cast it to " << T::s_class_name << std::endl;
    // std::cout << "- Cache object " << get_cache<T>(res.class_name()) << std::endl;
    // std::cout << "- Object " << get_cache<T>(res.class_name())->get(*this, res, read_children, read_children) << std::endl;
    ...
    try
      {
        ...
        for (auto& i : objs)
          {
            // std::cout << "+ Looking up " << i.UID() << " <" << i.class_name() << "> and cast it to " << T::s_class_name << std::endl;
            // std::cout << "+ Cache object " << get_cache<T>(i.class_name()) << std::endl;
            // std::cout << "+ Object " << get_cache<T>(i.class_name())->get(*this, i, read_children, read_children) << std::endl;
            results.push_back(get_cache<T>(i.class_name())->get(*this, i, read_children, read_children));
          }
      }
    catch (dunedaq::oksdbinterfaces::Generic & ex)
      {
        throw(dunedaq::oksdbinterfaces::Generic( ERS_HERE, mk_ref_ex_text("objects", T::s_class_name, name, obj).c_str(), ex ) );
      }
  }
...
template<class T, class V>
  ...
  Configuration::referenced_by(const T& obj, std::vector<const V*>& results, const std::string& relationship_name, bool check_composite_only, bool init, unsigned long rlevel, const std::vector<std::string> * rclasses)
  {
    ...
    try
      ...
      {
        throw(dunedaq::oksdbinterfaces::Generic( ERS_HERE, mk_ref_by_ex_text(V::s_class_name, relationship_name, obj.p_obj).c_str(), ex ) );
      }
  }
...
Configuration::Cache<T>::get(Configuration& oksdbinterfaces,
  ...
  {
    // std::cout << "AAAA 0" << obj.m_impl->m_id << " from " << m_class_name <<" to " << T::s_class_name << std::endl;
    ...
    // std::cout << "AAAA R" << result << std::endl;
    return result;
  }
...
template<class T>
  ...
  {
    oksdbinterfaces::map<std::list<AttributeConverterBase*> *>::const_iterator l = m_convert_map.find(typeid(T).name());
