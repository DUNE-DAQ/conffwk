...
template<class T>
const T*
Configuration::_get(const std::string& name,
                    bool init_children,
                    bool init_object,
...
template<class T>
void
Configuration::_get(std::vector<const T*>& result,
                    bool init_children,
                    bool init_object,
...
Configuration::_ref(ConfigObject& obj, const std::string& name, bool read_children)
{
  ...
  // return ((!res.is_null()) ? get_cache<T>(res.class_name())->get(*this, res, read_children, read_children) :
  // nullptr);
}
...
void
Configuration::referenced_by(const T& obj,
                             std::vector<const V*>& results,
                             ...
                             const std::vector<std::string>* rclasses)
{
  ...
  try {
    ...
    throw(dunedaq::conffwk::Generic(
      ERS_HERE, mk_ref_by_ex_text(V::s_class_name, relationship_name, obj.p_obj).c_str(), ex));
  }
}
...
// template<class T> T *
// Configuration::Cache<T>::get(Configuration& conffwk, const std::string& name, bool init_children, bool init_object,
// unsigned long rlevel, const std::vector<std::string> * rclasses)
// {
...
//       "\')"; c->m_cache[new_id] = it->second; c->m_cache.erase(it);
