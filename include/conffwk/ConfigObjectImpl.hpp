...
// Renamed since fork: yes (from config/ConfigObjectImpl.h to include/conffwk/ConfigObjectImpl.hpp).
//
...
public:
  ...
  ConfigObjectImpl(ConfigurationImpl* impl,
                   const std::string& id,
                   dunedaq::conffwk::ObjectState state = dunedaq::conffwk::Valid) noexcept;
...
public:
  ...
  const std::string& class_name() const noexcept { return *m_class_name; }
...
public:
  /// Virtual method to read any relationship value without throwing an exception if there is no such relationship
  /// (return false)
  ...
  virtual void referenced_by(std::vector<ConfigObject>& value,
                             const std::string& association,
                             bool check_composite_only,
                             unsigned long rlevel,
                             const std::vector<std::string>* rclasses) const = 0;
...
public:
  ...
  /// Virtual method to read vector-of-conffwk-objects relationship value
  virtual void set(const std::string& association,
