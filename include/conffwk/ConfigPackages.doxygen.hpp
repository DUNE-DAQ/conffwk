...
\li genconffwk.info file containing information about names of generated classes,
    the C++ code namespace, include prefix directory and java package name


  \subsection GenconffwkParams 2.1. Parameters of genconffwk utility
...
and in the conffwk package in Java. Below it %is described how to create an object of this class and how
...
then the configuration constructor throws \c daq::conffwk::Generic in case of C++,
or \c conffwk.SystemException %is thrown in case of Java.
...
\li Java methods generated in class T_Helper:
  <ul>
  <li> static public %T get(conffwk.Configuration db, String id) to read named object
  <li> static public %T[] gets(conffwk.Configuration db, Query query) to read objects of class
  </ul>
...
Note, if object with such ID does not exist, conffwk.NotFoundException exception %is thrown.
\code{.java}
try {
...
In case of Java a converter class has to implement  conffwk.AttributeConverter interface
...
\code{.java}
try {
  conffwk.Configuration db = new conffwk.Configuration("rdbconffwk");
...
\li <tt> %T create(conffwk.Configuration db, String at, String id) </tt> -
    to create new object of class %T with identity id at existing database file with name at;
\li <tt> %T create(conffwk.Configuration db, conffwk.DalObject at, String id) </tt> -
    to create new object of class %T with identity id at a database file where object at %is stored.
...
class TestCallback implements conffwk.Callback {
  private conffwk.Configuration db;
