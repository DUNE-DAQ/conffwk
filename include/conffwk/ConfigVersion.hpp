...
// Renamed since fork: yes (from config/ConfigVersion.h to include/conffwk/ConfigVersion.hpp).
//
...
public:
  ...
  Version(const std::string& id,
          ...
          const std::vector<std::string>& files) noexcept
    : m_id(id)
    , m_user(user)
    , m_timestamp(ts)
    , m_comment(comment)
...
public:
   ...
   *  \return timestamp
   */
  std::time_t get_timestamp() const noexcept { return m_timestamp; }

  /**
   *  The commit comment.
   *  \return user comment
   */
  const std::string& get_comment() const noexcept { return m_comment; }

  /**
   *  The files modified in this version.
   *  \return the modified files
   */
  const std::vector<std::string>& get_files() const noexcept { return m_files; }
