...
public:
  ...
  bool is_null() const noexcept { return (m_impl == nullptr); }

  /**
   *  \brief Check if object was deleted.
   */

  bool is_deleted() const { return (m_impl->is_deleted()); }
...
public:
  ...
  const std::string contained_in() const { return m_impl->contained_in(); }
   ...
   * object). For generic relationships the method performs full scan of all database objects, so at large scale this
   * method should not be applied to every object.
   *
   ...
   * referenced by given object have also to be read to the implementation cache)
  ...
  void move(const std::string& at) { m_impl->move(at); }
...
std::ostream&
operator<<(std::ostream&, const ConfigObject*);
