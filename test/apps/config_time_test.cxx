...
// Renamed since fork: yes (from test/config_time_test.cpp to test/apps/config_time_test.cxx).
//
...
ERS_DECLARE_ISSUE(conffwk_time_test, BadCommandLine, "bad command line: " << reason, ((const char*)reason))

ERS_DECLARE_ISSUE(conffwk_time_test, ConfigException, "caught dunedaq::conffwk::Exception exception", )
...
main(int argc, char* argv[])
{
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (!strcmp(cp, "-h") || !strcmp(cp, "--help")) {
      std::cout << "Usage: conffwk_time_test -d dbspec [-c | -C [class_name]] [-o | -O [object_id]] [-n]\n"
                   "\n"
                   "Options/Arguments:\n"
