...
// Renamed since fork: yes (from test/config_test_rw.cpp to test/apps/config_test_rw.cxx).
//
...
ERS_DECLARE_ISSUE(conffwk_test_rw, BadCommandLine, "bad command line: " << reason, ((const char*)reason))

ERS_DECLARE_ISSUE(conffwk_test_rw, ConfigException, "caught dunedaq::conffwk::Exception exception", )

static void
usage()
{
  std::cout << "Usage: conffwk_test_rw -d data_name -s schema_name -p plugin_spec\n"
               "\n"
               "Options/Arguments:\n"
...
check_object(ConfigObject& o, const std::string& name, ConfigObject* o1)
{
  ...
  if (o1 == 0) {
    ...
    if (o2.is_null()) {
      ...
      if (!(o2 == *o1)) {
        std::cerr << "ERROR reading relationship: \'" << name << "\' (read and wrote objects are different)"
                  << std::endl;
      }
    }
  }
...
void
check_file_path(ConfigObject& o, const std::string& file_name)
{
...
void
check_rename(ConfigObject& o, const std::string& name)
{
...
main(int argc, char* argv[])
{
  ...
  if (!plugin_name) {
    ers::fatal(
      conffwk_test_rw::BadCommandLine(ERS_HERE, "no plugin specification given (oksconflibs, rdbconffwk:server-name)"));
    return (EXIT_FAILURE);
  }
  ...
  try {
    ...
    INIT(int8_t, 0x7E, int8_values);
    std::vector<uint8_t> uint8_values;
    INIT(uint8_t, 0xFE, uint8_values);
    std::vector<int16_t> int16_values;
    INIT(int16_t, 0x7FFE, int16_values);
    std::vector<uint16_t> uint16_values;
    INIT(uint16_t, 0xFFFE, uint16_values);
    std::vector<int32_t> int32_values;
    INIT(int32_t, 0x7FFFFFFE, int32_values);
    std::vector<int32_t> uint32_values;
    INIT(uint32_t, 0xFFFFFFFE, uint32_values);
    std::vector<int64_t> int64_values;
    INIT(int64_t, (std::numeric_limits<int64_t>::max() - 2), int64_values);
    std::vector<uint64_t> uint64_values;
    INIT(uint64_t, (std::numeric_limits<uint64_t>::max() - 1), uint64_values);
    std::vector<float> float_values;
    INIT(float, 123.456, float_values);
    std::vector<double> double_values;
    INIT(double, 1234567890.123456, double_values);
    std::vector<std::string> strings_values;
    strings_values.push_back("test20");
    strings_values.push_back("test30");
    strings_values.push_back("test10");
    std::vector<std::string> enum_values;
    enum_values.push_back("THIRD");
    enum_values.push_back("SECOND");
    enum_values.push_back("FIRST");
    std::vector<std::string> class_values;
    class_values.push_back("Dummy");
    class_values.push_back("Second");
    class_values.push_back("Third");
    ...
    std::vector<const ::ConfigObject*> vec4;
    vec4.push_back(&o1);
    vec4.push_back(&o2);
    std::vector<const ::ConfigObject*> vec5;
    vec5.push_back(&o3);
    vec5.push_back(&o4);
    std::vector<const ::ConfigObject*> vec6;
    vec6.push_back(&o3);
    ...
    std::string f11(data_name);
    f11 += ".1.1";
    std::string f12(data_name);
    f12 += ".1.2";
    std::string f21(data_name);
    f21 += ".2.1";
    std::string f22(data_name);
    f22 += ".2.2";
    ...
    std::string f1(data_name);
    f1 += ".1";
    std::string f2(data_name);
    f2 += ".2";

    {
      std::list<std::string> includes(1, schema_name);
      includes.push_back(f11);
      includes.push_back(f12);
      db.create(f1, includes);
    }

    {
      std::list<std::string> includes(1, schema_name);
      includes.push_back(f21);
      includes.push_back(f22);
      includes.push_back(f12);
    ...
    struct
    {
      ...
      const char* id;
    } data[12] = { { &f1, "f1-1" },   { &f1, "f1-2" },   { &f2, "f2-1" },   { &f2, "f2-2" },
                   { &f11, "f11-1" }, { &f11, "f11-2" }, { &f12, "f12-1" }, { &f12, "f12-2" },
                   { &f21, "f21-1" }, { &f21, "f21-2" }, { &f22, "f22-1" }, { &f22, "f22-2" } };
    ...
    const char* removed_objects_by_include[] = {
      "f1-1", "f1-2", "f11-1", "f11-2"
    }; // these objects have to be removed (note, f12 is still included by f2)
      ...
      try {
        ...
        for (int j = 0; j < 4; ++j) {
          if (!strcmp(removed_objects_by_include[j], data[i].id)) {
            std::cout << "FAILED, object was not removed";
            state = true;
            break;
          }
        }
        ...
        for (int j = 0; j < 4; ++j) {
          if (!strcmp(removed_objects_by_include[j], data[i].id)) {
            std::cout << "OK, object was removed";
            state = true;
            break;
          }
        }
    ...
    const char* removed_objects_by_composite_parent[] = {
      "#4", "#5"
    }; // these objects have to be removed (note, #3 is still referenced by #6

    db.destroy_obj(o5);

    std::cout << "TEST deleted object " << o5.UID() << " existence: "
              << (o5.is_deleted() ? "OK (is_deleted returns TRUE)" : "FAILED (is_deleted returns FALSE)") << std::endl;
    ...
    std::cout << "TEST deleted object " << o4.UID() << " existence: " << (o4.is_deleted() ? "OK" : "FAILED")
              << std::endl;
    const std::string deleted_name(o4.UID());
    o6.rename(deleted_name);
    check_rename(o6, deleted_name);
    std::cout << "TEST deleted object " << deleted_name
              << " after renamed existing object to it's ID: " << (!o4.is_deleted() ? "OK" : "FAILED") << std::endl;
