...
{
  if (this == &other || m_impl == other.m_impl)
    return true; // the objects or implementations are the same
  if (!m_impl || !other.m_impl)
    return false; // only one of objects has no implementation
...
void
ConfigObject::print_ref(std::ostream& s,
                        Configuration& conffwk,
                        const std::string& prefix,
                        bool show_contained_in) const noexcept
{
  static bool expand_aggregation = (getenv(
    "TDAQ_CONFFWK_PRINT_EXPAND_AGGREGATIONS")); // FIXME tdaq-09-05-00 => add new parameter to conffwk and add fuse
  ...
  try {
    ...
    for (const auto& i : cd.p_attributes) {
      ...
      switch (i.p_type) {
        ...
        case dunedaq::conffwk::class_type:
          print_value<std::string>(*this, aname, ismv, '\"', s);
          break;
        case dunedaq::conffwk::bool_type:
          print_value<bool>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::u8_type:
          print_value<uint8_t>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::s8_type:
          print_value<int8_t>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::u16_type:
          print_value<uint16_t>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::s16_type:
          print_value<int16_t>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::u32_type:
          print_value<uint32_t>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::s32_type:
          print_value<int32_t>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::u64_type:
          print_value<uint64_t>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::s64_type:
          print_value<int64_t>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::float_type:
          print_value<float>(*this, aname, ismv, 0, s);
          break;
        case dunedaq::conffwk::double_type:
          print_value<double>(*this, aname, ismv, 0, s);
          break;
        default:
          s << "*** bad type ***";
      }
    ...
    for (const auto& i : cd.p_relationships) {
      ...
      if (expand_aggregation == false || i.p_is_aggregation == false) {
        s << ' ';
        print_value<ConfigObject>(*this,
                                  i.p_name,
                                  (i.p_cardinality == dunedaq::conffwk::zero_or_many) ||
                                    (i.p_cardinality == dunedaq::conffwk::one_or_many),
                                  '\"',
                                  s);
        s << std::endl;
      } else {
