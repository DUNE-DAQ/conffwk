...
DalObject::get_rel_objects(const std::string& name, bool upcast_unregistered, std::vector<const DalObject*>& objs) const
{
  ...
  if (const_cast<conffwk::ConfigObject*>(&p_obj)->rel(name, c_objs)) {
    ...
    p_registry.get(c_objs, upcast_unregistered).swap(objs);
    return true;
  }
...
DalObject::get_algo_objects(const std::string& /* name */, std::vector<const DalObject*>& /* objs */) const
{
  ...
  //   TLOG_DEBUG(2) << "suitable class for algorithm " << name << " on object " << this << " is " <<
  //   suitable_dal_class;

  //   if (!suitable_dal_class.empty())
  //     if (const DalObject *obj = p_registry.make_dal_object(const_cast<conffwk::ConfigObject&>(p_obj), UID(),
  //     suitable_dal_class))
  //       {
...
void
DalObject::throw_init_ex(dunedaq::conffwk::Exception& ex)
{
...
void
DalObject::throw_get_ex(const std::string& what, const std::string& class_name, const DalObject* obj)
{
  std::ostringstream text;
  text << "cannot find relationship or algorithm \"" << what << "\" in c++ class \"" << class_name << "\" for object "
       << obj;
  throw dunedaq::conffwk::Generic(ERS_HERE, text.str().c_str());
}
