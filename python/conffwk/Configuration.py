...
# Renamed since fork: yes (from python/config/Configuration.py to python/conffwk/Configuration.py).
...
class Configuration(_ConfigurationProxy):
...
to see if there's a problem with the input database""")
    ...
    def relations(self, class_name, all=False):
        ...
        Raises RuntimeError on problems
        """
        relationship_properties_as_strings = super(Configuration, self).relations(class_name, all)
        relationship_properties_to_return = {}

        for relationship, properties in relationship_properties_as_strings.items():
            ...
            relationship_properties_to_return[relationship] = properties

        return relationship_properties_to_return
